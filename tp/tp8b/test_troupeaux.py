import troupeaux
mon_troupeau = troupeaux.mon_troupeau
...
def test_total_animaux():
    ...
    assert troupeaux.total_animaux(mon_troupeau) == 20
...
def test_tous_les_animaux():
    ...
    assert troupeaux.tous_les_animaux(mon_troupeau) == {'vache', 'mouton', 'chat'}
...
def test_reunion_troupeaux():
    ...
    assert troupeaux.reunion_troupeaux(troupeau_de_perrette, mon_troupeau) == {'veau': 14, 'vache': 21, 'poule': 42, 'mouton': 2, 'chat': 4}
    assert troupeaux.reunion_troupeaux(mon_troupeau, troupeau_de_jean) == {'vache': 26, 'mouton': 2, 'chat': 4, 'cochon': 17, 'veau': 3}
